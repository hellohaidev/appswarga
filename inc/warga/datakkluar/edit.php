...
if(isset($_POST['update'])){
    ...
    $queryUpdate = mysqli_query($link,"UPDATE tm_data_kk_luar
                                    SET tm_data_kk_luar.nama_kk='$nama_kk',
                                    tm_data_kk_luar.tempat_lahir='$tempat_lahir',
                                    tm_data_kk_luar.tgl_lahir='$tgl_lahir',
                                    tm_data_kk_luar.jk='$jk',
                                    tm_data_kk_luar.agama='$agama',
                                    tm_data_kk_luar.pendidikan='$pendidikan',
                                    tm_data_kk_luar.pekerjaan='$pekerjaan',
                                    tm_data_kk_luar.nokk='$nokk',
                                    tm_data_kk_luar.ktp='$ktp',
                                    tm_data_kk_luar.jml_anggota='$jml_anggota',
                                    tm_data_kk_luar.alamat='$alamat',
                                    tm_data_kk_luar.tetap='$tetap',
                                    tm_data_kk_luar.luar='$luar',
                                    tm_data_kk_luar.kontrak='$kontrak'
                                    WHERE id_kk_luar = '$id' ");
    if($queryUpdate){
        echo '<script>alert("ok");window.location.assign("http://localhost/wargaapps/page.php?page=homekkluar")</script>';
    }
    else {
        echo '<script>alert("gagal");window.location.assign("http://localhost/wargaapps/page.php?page=homekkluar")</script>';
    }
}
else {

$query = "SELECT * FROM  tm_data_kk_luar where id_kk_luar = '$id'";
