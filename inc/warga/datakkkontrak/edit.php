...
if(isset($_POST['update'])){
    ...
    $queryUpdate = mysqli_query($link,"UPDATE tm_data_kk_kontrak
                                    SET tm_data_kk_kontrak.nama_kk='$nama_kk',
                                    tm_data_kk_kontrak.tempat_lahir='$tempat_lahir',
                                    tm_data_kk_kontrak.tgl_lahir='$tgl_lahir',
                                    tm_data_kk_kontrak.jk='$jk',
                                    tm_data_kk_kontrak.agama='$agama',
                                    tm_data_kk_kontrak.pendidikan='$pendidikan',
                                    tm_data_kk_kontrak.pekerjaan='$pekerjaan',
                                    tm_data_kk_kontrak.nokk='$nokk',
                                    tm_data_kk_kontrak.ktp='$ktp',
                                    tm_data_kk_kontrak.jml_anggota='$jml_anggota',
                                    tm_data_kk_kontrak.alamat='$alamat',
                                    tm_data_kk_kontrak.tetap='$tetap',
                                    tm_data_kk_kontrak.luar='$luar',
                                    tm_data_kk_kontrak.kontrak='$kontrak'
                                    WHERE id_kk_kontrak = '$id' ");
    if($queryUpdate){
        echo '<script>alert("ok");window.location.assign("http://localhost/wargaapps/page.php?page=homekkkontrak")</script>';
    }
    else {
        echo '<script>alert("gagal");window.location.assign("http://localhost/wargaapps/page.php?page=homekkkontrak")</script>';
    }
}
else {

$query = "SELECT * FROM  tm_data_kk_kontrak where id_kk_kontrak = '$id'";
