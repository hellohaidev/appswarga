...
if(isset($_POST['update'])){
    ...
    $queryUpdate = mysqli_query($link,"UPDATE tm_data_kk_tetap
                                    SET tm_data_kk_tetap.nama_kk='$nama_kk',
                                    tm_data_kk_tetap.tempat_lahir='$tempat_lahir',
                                    tm_data_kk_tetap.tgl_lahir='$tgl_lahir',
                                    tm_data_kk_tetap.jk='$jk',
                                    tm_data_kk_tetap.agama='$agama',
                                    tm_data_kk_tetap.pendidikan='$pendidikan',
                                    tm_data_kk_tetap.pekerjaan='$pekerjaan',
                                    tm_data_kk_tetap.nokk='$nokk',
                                    tm_data_kk_tetap.ktp='$ktp',
                                    tm_data_kk_tetap.jml_anggota='$jml_anggota',
                                    tm_data_kk_tetap.alamat='$alamat',
                                    tm_data_kk_tetap.tetap='$tetap',
                                    tm_data_kk_tetap.luar='$luar',
                                    tm_data_kk_tetap.kontrak='$kontrak'
                                    WHERE id_kk_tetap = '$id' ");
    if($queryUpdate){
        echo '<script>alert("ok");window.location.assign("http://localhost/wargaapps/page.php?page=homekktetap")</script>';
    }
    else {
        echo '<script>alert("gagal");window.location.assign("http://localhost/wargaapps/page.php?page=homekktetap")</script>';
    }
}
else {

$query = "SELECT * FROM  tm_data_kk_tetap where id_kk_tetap = '$id'";
